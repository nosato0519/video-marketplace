...
async function seedBuyerPurchase() {
  ...
  await pool.query(
    ...
    [ids.seller, `seller-${ids.seller}@acceptance.test`, ids.buyer, buyerEmail]
  );
  ...
  await pool.query(
    `INSERT INTO products
     ...
     VALUES ($1, $2, $3, 'published', 1500, 'JPY', 'Browser E2E Product', 'Real backend browser acceptance product', TRUE, TRUE, NOW())`,
    [ids.product, ids.seller, ids.media]
  );
  await pool.query(
    `INSERT INTO user_sessions (user_id, token_hash, expires_at)
     VALUES ($1, $2, $3)`,
    [ids.buyer, hashSessionToken(buyerToken), sessionExpiry()]
  );
}
...
async function settleOrder(orderId) {
  const payment = await pool.query(
    `INSERT INTO payments
     ...
     RETURNING id`,
    [crypto.randomUUID(), orderId, ids.buyer, providerPaymentId, `mock:${providerPaymentId}`]
  );
  ...
  const response = await fetch('http://127.0.0.1:3000/api/payments/webhook', {
    method: 'POST',
...
test.describe('buyer real-backend browser acceptance', () => {
  test.beforeAll(seedBuyerPurchase);
  test.afterAll(async () => {
  ...
  test('browse to product detail uses the real catalog and starts a real order', async ({ page, context }) => {
    ...
    await expect(card).toContainText('Browser E2E Product');
    await card.click();
    ...
    expect(order.order.product_id).toBe(ids.product);
    await settleOrder(order.order.id);
  ...
  test('watch and download remain protected by the real entitlement', async ({ page, context }) => {
    await context.addCookies([{ name: 'video_marketplace_session', value: buyerToken, domain: '127.0.0.1', path: '/' }]);
    const order = await pool.query(
      `SELECT id FROM orders WHERE buyer_id = $1 AND product_id = $2 ORDER BY created_at DESC LIMIT 1`,
      [ids.buyer, ids.product]
    );
    ...
    const download = await page.request.get(`/api/media/${ids.product}/download`, {
      headers: { cookie: `video_marketplace_session=${encodeURIComponent(buyerToken)}` },
    });
