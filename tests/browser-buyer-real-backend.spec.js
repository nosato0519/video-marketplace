...
test.describe('buyer real-backend browser acceptance', () => {
  ...
  test('browse, open a real product, create an order, then access library', async ({ page, context }) => {
  ...
  test('protected watch and download work for the real entitled buyer', async ({ page, context }) => {
    await context.addCookies([{ name:'video_marketplace_session', value:buyerToken, domain:'127.0.0.1', path:'/' }]);
