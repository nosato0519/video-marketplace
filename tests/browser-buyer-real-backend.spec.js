...
process.env.DATABASE_URL ||= 'postgresql://postgres:postgres@127.0.0.1:5432/video_marketplace_test';
...
test.describe('buyer real-backend browser acceptance', () => {
  ...
  test('browse, open real product, create real order, settle it, and access Library', async ({ page, context }) => {
    ...
    await Promise.all([orderResponsePromise, page.getByRole('button',{name:'Purchase'}).click()]);
