...
test.describe('buyer purchase browser acceptance', () => {
  ...
  test('buyer can open a product and start secure checkout', async ({ page }) => {
    ...
    await page.route('**/api/catalog/products/demo-1/purchase-intent', async (route) => {
      ...
      await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ data: { price: 12.99, currency: 'USD' } }) });
    });
    ...
    await page.route('**/api/orders', async (route) => {
      ...
      orderCreated = true;
      await route.fulfill({ status: 201, contentType: 'application/json', body: JSON.stringify({ order: { id: 'order-1', status: 'pending', product_id: 'demo-1' } }) });
    });

    await page.route('**/api/orders/order-1/checkout', async (route) => {
      expect(route.request().method()).toBe('POST');
      checkoutStarted = true;
      await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ session: { status: 'ready', url: '/mock-checkout/session-1' } }) });
    });

    await page.route('**/mock-checkout/session-1', async (route) => {
      await route.fulfill({ status: 200, contentType: 'text/html', body: '<!doctype html><html><body><h1>Test Checkout</h1><p>Secure payment session</p></body></html>' });
    });
  ...
  test('buyer library exposes protected watch and download actions', async ({ page }) => {
    ...
    await page.route('**/api/library', async (route) => {
      await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ items: [{ product_id: 'demo-1', title: 'Featured Video', description: 'Purchased video product.', streaming_enabled: true, download_enabled: true }] }) });
    });
    await page.goto(appUrl('#/library'));
