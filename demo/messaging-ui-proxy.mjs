...
const NAV_STYLE = `<style id="message-nav-ui">
.message-nav-card{margin-top:18px;border:1px solid #35333a;background:linear-gradient(145deg,#15161a,#0d0e11);padding:22px}.message-nav-card h3{margin:0;font-size:16px;letter-spacing:-.02em}.message-nav-card p{margin:7px 0 16px;color:#888;font-size:9px;line-height:1.7}.message-feature-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.message-feature-card{display:flex;flex-direction:column;min-height:156px;border:1px solid #35333a;background:linear-gradient(145deg,#111216,#0b0c0f);padding:18px;text-decoration:none;color:#eee;transition:border-color .18s,transform .18s,background .18s}.message-feature-card:hover{border-color:#c6a864;background:linear-gradient(145deg,#17150f,#0d0e11);transform:translateY(-1px)}.message-feature-icon{width:36px;height:36px;border:1px solid #5b4b2d;display:grid;place-items:center;color:#d9b45f;font-size:17px;margin-bottom:13px}.message-feature-card h4{margin:0 0 7px;font-size:13px}.message-feature-card p{margin:0 0 13px;color:#99958f;font-size:9px;line-height:1.7}.message-feature-meta{display:flex;gap:7px;flex-wrap:wrap;margin-top:auto}.message-feature-badge{display:inline-flex;padding:5px 8px;border:1px solid #3e3d43;color:#bcb7ae;font-size:8px}.message-feature-badge.alert{border-color:#5b4b2d;color:#d9b45f}.message-feature-action{margin-top:13px;color:#c6a864;font-size:9px;letter-spacing:.04em}@media(max-width:760px){.message-feature-grid{grid-template-columns:1fr}.message-feature-card{min-height:140px}}
</style>`;
...
function injectNavigation(html, pathname){
  const cards = pathname==='/pages/creator-studio.html' ? `${NAV_STYLE}<section class="message-nav-card"><h3>メッセージ・運営サポート</h3><p>購入者とのやり取りと、運営への問い合わせをここから管理できます。</p><div class="message-feature-grid"><a class="message-feature-card" href="/pages/messages.html?role=seller"><span class="message-feature-icon">💬</span><h4>メッセージ</h4><p>購入者からの商品・動画に関する問い合わせを確認し、返信できます。</p><div class="message-feature-meta"><span class="message-feature-badge alert">未返信 1件</span><span class="message-feature-badge">受信 2件</span></div><span class="message-feature-action">メッセージを管理 →</span></a><a class="message-feature-card" href="/pages/support.html"><span class="message-feature-icon">✉</span><h4>運営への問い合わせ</h4><p>審査・売上・振込・アカウントなどについて運営へ問い合わせできます。</p><div class="message-feature-meta"><span class="message-feature-badge alert">対応中 1件</span><span class="message-feature-badge">履歴 2件</span></div><span class="message-feature-action">問い合わせを管理 →</span></a></div></section>` : pathname==='/pages/account.html' ? `${NAV_STYLE}<section class="message-nav-card"><h3>販売者へのメッセージ</h3><p>購入した動画について販売者へ質問・確認を送れます。</p><a href="/pages/messages.html?role=buyer">メッセージを確認・送信 →</a></section>` : /product-detail\.html$/.test(pathname) ? `${NAV_STYLE}<section class="message-nav-card"><h3>販売者へ質問する</h3><p>この動画について販売者に質問・確認ができます。</p><a href="/pages/messages.html?role=buyer&product=動画についての質問">販売者にメッセージを送る →</a></section>` : /admin/i.test(pathname) ? `${NAV_STYLE}<section class="message-nav-card"><h3>問い合わせ管理</h3><p>販売者からの問い合わせを確認し、返信・対応状況を管理できます。</p><a href="/pages/admin-support.html">問い合わせ管理を開く →</a></section>` : '';
