...
function finalize(html) {
  ...
  if (hero && hero.index != null) {
    ...
    const afterHero = html.slice(at);
    const trust = afterHero.match(/<section\b[^>]*class=["'][^"']*\btrustbar\b[^"']*["'][^>]*>[\s\S]*?<\/section>/i);
    if (trust && trust.index != null) {
      const trustEnd = at + trust.index + trust[0].length;
      html = html.slice(0, at) + SYSTEM_BLOCK + html.slice(at, trustEnd) + html.slice(trustEnd);
    } else {
