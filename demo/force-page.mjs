...
function finalize(html) {
  ...
  const orderedTop = [hero.section, SYSTEM_BLOCK, platformTake.section, showcaseTake.section, trustTake.section, popularTake.section, spotlightTake.section, collectionsTake.section, genreRailTake.section, recommendationsTake.section].filter(Boolean).join('');
  return (orderedTop + html).replace('</head>', STYLE + '</head>');
}
  ...
  const proxyReq = httpRequest({hostname:'127.0.0.1',port:upstreamPort,path:req.url,method:req.method,headers:req.headers}, proxyRes => {
    ...
    if (req.method === 'GET' && (req.url === '/' || req.url.startsWith('/index.html')) && type.includes('text/html')) {
      ...
      proxyRes.on('end', () => {
        ...
        const headers={...proxyRes.headers,'content-length':Buffer.byteLength(html),'cache-control':'no-store','x-demo-version':'20260907-v26'};
