...
function finalize(html) {
  ...
  const hero = takeSection(html, '<section class="hero"'); html = hero.html;
  const platformTake = takeSection(html, '<section class="platform"'); html = platformTake.html;
  const showcaseTake = takeSection(html, '<section class="video-showcase"'); html = showcaseTake.html;
  const trustTake = takeSection(html, '<section class="trustbar"'); html = trustTake.html;
  const popularTake = takeSection(html, '<section class="section" id="videos"'); html = popularTake.html;
  const spotlightTake = takeSection(html, '<section class="spotlight"'); html = spotlightTake.html;
  const collectionsTake = takeSection(html, '<section class="video-collections"'); html = collectionsTake.html;
  const genreRailTake = takeSection(html, '<section class="genre-rail"'); html = genreRailTake.html;
  const recommendationsTake = takeSection(html, '<section class="genre-recommendations"'); html = recommendationsTake.html;

  const orderedSections = [hero.section, SYSTEM_BLOCK, platformTake.section, showcaseTake.section, trustTake.section, popularTake.section, spotlightTake.section, collectionsTake.section, genreRailTake.section, recommendationsTake.section].filter(Boolean).join('');
  const bodyMatch = html.match(/<body\\b[^>]*>/i);
  if (!bodyMatch) return (orderedSections + html).replace('</head>', STYLE + '</head>');
  const bodyStart = bodyMatch.index + bodyMatch[0].length;
  const bodyEnd = html.lastIndexOf('</body>');
  if (bodyEnd < bodyStart) return (orderedSections + html).replace('</head>', STYLE + '</head>');
  const beforeBody = html.slice(0, bodyStart);
  const afterBody = html.slice(bodyEnd);
  return (beforeBody + orderedSections + afterBody).replace('</head>', STYLE + '</head>');
}
...
createServer((req, res) => {
  const proxyReq = httpRequest({hostname:'127.0.0.1',port:upstreamPort,path:req.url,method:req.method,headers:req.headers}, proxyRes => {
    ...
    if (req.method === 'GET' && (req.url === '/' || req.url.startsWith('/index.html')) && type.includes('text/html')) {
      ...
      proxyRes.on('end', () => {
        ...
        const headers={...proxyRes.headers,'content-length':Buffer.byteLength(html),'cache-control':'no-store','x-demo-version':'20260907-v27'};
  ...
  proxyReq.on('error', err => { res.writeHead(502, {'content-type':'text/plain; charset=utf-8'}); res.end(`proxy error: ${err.message}`); });
  req.pipe(proxyReq);
});
