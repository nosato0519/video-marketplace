...
try {
  ...
  await pool.query(
    ...
    [ids.seller, `seller-${ids.seller}@acceptance.test`, ids.otherSeller, `other-seller-${ids.otherSeller}@acceptance.test`]
  );
  ...
  await pool.query(
    `INSERT INTO user_sessions (user_id, token_hash, expires_at)
     VALUES ($1, $2, $3), ($4, $5, $6)`,
    [ids.seller, hashSessionToken(sellerToken), sessionExpiry(), ids.otherSeller, hashSessionToken(otherSellerToken), sessionExpiry()]
  );

  const uploadResponse = await request(baseUrl, '/api/seller/media/upload', {
    method: 'POST',
    headers: {
      cookie: sellerCookie,
      'content-type': 'video/mp4',
      'content-length': String(uploadBytes.length),
      'x-original-filename': 'seller-e2e.mp4',
    },
    body: uploadBytes,
  });
  ...
  const createResponse = await request(baseUrl, '/api/seller/products', {
    method: 'POST',
    headers: { 'content-type': 'application/json', cookie: sellerCookie },
    body: JSON.stringify({
      title: 'Seller E2E Product',
      description: 'Seller product/media end-to-end acceptance',
      priceAmount: 2200,
      priceCurrency: 'JPY',
  ...
  const patchResponse = await request(baseUrl, `/api/seller/products/${ids.product}`, {
    method: 'PATCH',
    headers: { 'content-type': 'application/json', cookie: sellerCookie },
    body: JSON.stringify({ description: 'Updated seller E2E product description' }),
  });
  ...
  const publishResponse = await request(baseUrl, `/api/seller/products/${ids.product}/publish`, {
    method: 'POST',
  ...
  assert.equal(otherDetailResponse.response.status, 403, JSON.stringify(otherDetailResponse.body));

  const lockedUpdateResponse = await request(baseUrl, `/api/seller/products/${ids.product}`, {
    method: 'PATCH',
    headers: { 'content-type': 'application/json', cookie: sellerCookie },
