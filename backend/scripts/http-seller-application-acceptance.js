...
try {
  ...
  const invalid = await request(baseUrl, '/api/seller/application', {
    method: 'POST',
    headers: { cookie: buyerCookie, 'content-type': 'application/json' },
    body: JSON.stringify({ displayName: '', legalName: '', countryCode: 'JPN' })
  });
  assert.equal(invalid.response.status, 400, JSON.stringify(invalid.body));

  const application = await request(baseUrl, '/api/seller/application', {
    method: 'POST',
    headers: { cookie: buyerCookie, 'content-type': 'application/json' },
    body: JSON.stringify({
      displayName: 'Acceptance Creator',
      legalName: 'Acceptance Creator Legal',
      countryCode: 'JP',
      message: 'I want to sell my original video products.'
    })
  });
  ...
  const duplicate = await request(baseUrl, '/api/seller/application', {
    method: 'POST',
    headers: { cookie: buyerCookie, 'content-type': 'application/json' },
    body: JSON.stringify({ displayName: 'Duplicate', legalName: 'Duplicate Legal', countryCode: 'JP' })
  });
  ...
  const adminRegister = await request(baseUrl, '/api/auth/register', {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
  ...
  const review = await request(baseUrl, `/api/admin/seller-applications/${applicationId}/review`, {
    method: 'POST',
    headers: { cookie: adminCookie, 'content-type': 'application/json' },
    body: JSON.stringify({ action: 'start_review', note: 'Initial review' })
  });
  ...
  const approve = await request(baseUrl, `/api/admin/seller-applications/${applicationId}/review`, {
    method: 'POST',
    headers: { cookie: adminCookie, 'content-type': 'application/json' },
    body: JSON.stringify({ action: 'approve', note: 'Approved by acceptance test' })
  });
  ...
  const audit = await query(
    `SELECT action, metadata FROM audit_events WHERE resource_type = 'seller_application' AND resource_id = $1 ORDER BY created_at ASC`,
    [applicationId]
  );
  ...
  const sellerLogin = await request(baseUrl, '/api/auth/login', {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ email: buyerEmail, password })
  });
  ...
  const secondRegister = await request(baseUrl, '/api/auth/register', {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ email: secondBuyerEmail, password })
  });
  assert.equal(secondRegister.response.status, 201, JSON.stringify(secondRegister.body));
  const secondLogin = await request(baseUrl, '/api/auth/login', {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ email: secondBuyerEmail, password })
  });
  const secondCookie = secondLogin.cookie;
  const secondApplication = await request(baseUrl, '/api/seller/application', {
    method: 'POST',
    headers: { cookie: secondCookie, 'content-type': 'application/json' },
    body: JSON.stringify({ displayName: 'Reject Creator', legalName: 'Reject Creator Legal', countryCode: 'JP' })
  });
  ...
  const reject = await request(baseUrl, `/api/admin/seller-applications/${secondApplicationId}/review`, {
    method: 'POST',
    headers: { cookie: adminCookie, 'content-type': 'application/json' },
  ...
  const rejectWithNote = await request(baseUrl, `/api/admin/seller-applications/${secondApplicationId}/review`, {
    method: 'POST',
    headers: { cookie: adminCookie, 'content-type': 'application/json' },
    body: JSON.stringify({ action: 'reject', note: 'Acceptance rejection fixture' })
  });
  ...
  const nonAdminReview = await request(baseUrl, `/api/admin/seller-applications/${secondApplicationId}/review`, {
    method: 'POST',
    headers: { cookie: sellerLogin.cookie, 'content-type': 'application/json' },
