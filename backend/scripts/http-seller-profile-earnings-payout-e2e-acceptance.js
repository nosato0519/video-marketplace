...
try {
  ...
  const profile = await request(baseUrl, '/api/seller/profile', {
    method: 'PUT',
    headers: { cookie, 'content-type': 'application/json' },
    body: JSON.stringify({ display_name: 'Acceptance Seller', bio: 'seller acceptance profile' })
  });
  ...
  const payout = await request(baseUrl, '/api/seller/payouts', {
    method: 'POST',
    headers: { cookie, 'content-type': 'application/json' },
  ...
  const concurrentPayout = await request(baseUrl, '/api/seller/payouts', {
    method: 'POST',
    headers: { cookie, 'content-type': 'application/json' },
    body: JSON.stringify({ amount: 2500, currency: 'JPY' })
  });
  ...
  const role = await request(baseUrl, `/api/admin/users/${adminUserId}/role`, {
    method: 'POST',
  ...
  assert.deepEqual(
    ...
    ['paid', 'processing', 'approved', 'reviewing']
  );
