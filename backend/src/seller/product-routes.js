...
router.get('/products', async (req, res, next) => {
  try {
    const result = await query(
      `SELECT id, seller_id, media_asset_id, status, price_amount, price_currency,
              title, description, created_at, updated_at, published_at
         FROM products
        WHERE seller_id = $1
...
router.post('/products', async (req, res, next) => {
  try {
    ...
    const result = await query(
      `INSERT INTO products (seller_id, media_asset_id, status, price_amount, price_currency, title, description)
       VALUES ($1, $2, 'draft', $3, $4, $5, $6)
       RETURNING id, seller_id, media_asset_id, status, price_amount, price_currency, title, description, created_at, updated_at, published_at`,
      [req.user.id, mediaAssetId, priceAmount, String(priceCurrency).toUpperCase(), String(title), String(description)]
    );
    ...
    const result = await query(
      `UPDATE products
          SET title = COALESCE($2, title),
              ...
              media_asset_id = COALESCE($6, media_asset_id),
              updated_at = NOW()
        WHERE id = $1 AND seller_id = $7
        RETURNING id, seller_id, media_asset_id, status, price_amount, price_currency, title, description, created_at, updated_at, published_at`,
      [req.params.productId, title == null ? null : String(title), description == null ? null : String(description), priceAmount == null ? null : priceAmount, priceCurrency == null ? null : String(priceCurrency).toUpperCase(), mediaAssetId == null ? null : mediaAssetId, req.user.id]
    );
    return res.json({ product: result.rows[0] });
  } catch (error) {
    return next(error);
...
router.post('/products/:productId/publish', async (req, res, next) => {
  try {
    const result = await query(
      `SELECT p.*, m.owner_user_id AS media_owner_user_id, m.status AS media_status
         FROM products p
    ...
    const validation = validateProductForPublishing({
      product: { title: row.title, price: row.price_amount, currency: row.price_currency },
      mediaAssets: row.media_asset_id ? [{ status: row.media_status }] : [],
    });
    if (row.media_owner_user_id && row.media_owner_user_id !== req.user.id) validation.errors.push('media_owner_mismatch');
    if (!validation.allowed || validation.errors.length) return res.status(422).json({ error: 'product_not_publishable', reasons: validation.errors });
    const published = await query(
      `UPDATE products
          SET status = 'published', published_at = COALESCE(published_at, NOW()), updated_at = NOW()
        WHERE id = $1 AND seller_id = $2 AND status IN ('draft','processing','submitted','under_review','approved')
        RETURNING id, seller_id, media_asset_id, status, price_amount, price_currency, title, description, created_at, updated_at, published_at`,
      [req.params.productId, req.user.id]
    );
...
router.post('/products/:productId/unpublish', async (req, res, next) => {
  try {
    const result = await query(
      `UPDATE products SET status = 'draft', published_at = NULL, updated_at = NOW()
