...
router.get('/application', async (req, res, next) => {
  try {
    const result = await query(`SELECT id, status, display_name, legal_name, country_code, message, review_note, submitted_at, reviewed_at, reviewed_by, created_at, updated_at FROM seller_applications WHERE user_id = $1 ORDER BY created_at DESC LIMIT 1`, [req.user.id]);
...
router.post('/application', async (req, res, next) => {
  try {
    ...
    const result = await query(`INSERT INTO seller_applications (user_id, display_name, legal_name, country_code, message) VALUES ($1, $2, $3, $4, $5) RETURNING id, status, display_name, legal_name, country_code, message, review_note, submitted_at, reviewed_at, reviewed_by, created_at, updated_at`, [req.user.id, displayName, legalName, countryCode, message]);
...
router.post('/application/withdraw', async (req, res, next) => {
  try {
    const result = await query(`UPDATE seller_applications SET status = 'withdrawn', updated_at = NOW() WHERE user_id = $1 AND status IN ('pending','under_review') RETURNING id, status, updated_at`, [req.user.id]);
