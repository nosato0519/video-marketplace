...
router.get('/payouts', async (req, res, next) => {
  try {
    const result = await query(
      `SELECT id, amount, currency, status, failure_reason,
...
router.post('/payouts', async (req, res, next) => {
  try {
    ...
    const balance = await query(
      `SELECT COALESCE(SUM(net_amount) FILTER (WHERE status = 'available'), 0) AS available
         FROM seller_earnings WHERE seller_id = $1 AND currency = $2`,
    ...
    const pending = await query(
      `SELECT COALESCE(SUM(amount), 0) AS pending
         FROM payouts WHERE seller_id = $1 AND currency = $2
          AND status IN ('requested','reviewing','approved','processing')`,
      [req.user.id, currency]
    );
    const pendingAmount = Number(pending.rows[0]?.pending || 0);
    ...
    const result = await query(
      `INSERT INTO payouts (seller_id, amount, currency, status)
       VALUES ($1, $2, $3, 'requested')
       RETURNING id, amount, currency, status, requested_at`,
      [req.user.id, amount.toFixed(2), currency]
    );
