...
export function validateMediaSecurityConfig(env = process.env) {
  ...
  if (provider === 's3') {
    if (!env.MEDIA_S3_BUCKET) throw new Error('media_s3_bucket_missing');
    if (!env.MEDIA_S3_ACCESS_KEY_ID) throw new Error('media_s3_access_key_missing');
    if (!env.MEDIA_S3_SECRET_ACCESS_KEY) throw new Error('media_s3_secret_key_missing');
    if (env.MEDIA_URL_SECRET && env.MEDIA_URL_SECRET.length < MIN_SECRET_LENGTH) {
      throw new Error('media_url_secret_too_short');
    }
    return true;
