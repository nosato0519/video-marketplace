...
const allowedTransitions = {
  ...
  reviewing: new Set(['approved','failed','cancelled']),
...
router.get('/payouts', async (_req, res, next) => {
  try {
    const result = await query(
      `SELECT p.id, p.seller_id, p.amount, p.currency, p.status,
              p.failure_reason, p.requested_at, p.reviewed_at, p.paid_at,
...
router.post('/payouts/:id/status', async (req, res, next) => {
  try {
    ...
    if (!allowedTransitions[nextStatus] && !Object.hasOwn(allowedTransitions, nextStatus)) {
      return res.status(400).json({ error: 'invalid_status' });
    }
    const current = await query(`SELECT id, status FROM payouts WHERE id = $1 FOR UPDATE`, [req.params.id]);
    ...
    const reason = req.body?.failure_reason ? String(req.body.failure_reason).slice(0, 500) : null;
    const result = await query(
      `UPDATE payouts
          SET status = $2,
              failure_reason = CASE WHEN $2 = 'failed' THEN $3 ELSE failure_reason END,
              reviewed_at = CASE WHEN $2 IN ('reviewing','approved','failed','cancelled') THEN COALESCE(reviewed_at, NOW()) ELSE reviewed_at END,
