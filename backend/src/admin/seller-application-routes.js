...
router.get('/seller-applications', async (req, res, next) => {
  try {
    ...
    const result = await query(
      `SELECT a.id, a.user_id, u.email, a.status, a.display_name, a.legal_name,
              a.country_code, a.message, a.review_note, a.submitted_at, a.reviewed_at,
              a.reviewed_by, a.created_at, a.updated_at
         FROM seller_applications a JOIN users u ON u.id = a.user_id
        WHERE a.status = $1 ORDER BY a.submitted_at ASC LIMIT 200`, [status]
    );
  ...
  try {
    ...
    const result = await withTransaction(async (client) => {
      ...
      const updated = await client.query(
        `UPDATE seller_applications SET status = $2, review_note = $3, reviewed_at = NOW(), reviewed_by = $4, updated_at = NOW()
          WHERE id = $1 RETURNING id, user_id, status, display_name, legal_name, country_code, message, review_note,
          submitted_at, reviewed_at, reviewed_by, updated_at`,
        [application.id, target, note, req.user.id]
      );
      if (target === 'approved') {
        await client.query(`UPDATE users SET role = 'seller', updated_at = NOW() WHERE id = $1 AND role = 'buyer'`, [application.user_id]);
        await client.query(
          `INSERT INTO seller_profiles (user_id, display_name, legal_name, country_code) VALUES ($1, $2, $3, $4)
           ON CONFLICT (user_id) DO UPDATE SET display_name = EXCLUDED.display_name, legal_name = EXCLUDED.legal_name,
      ...
      await client.query(
        `INSERT INTO audit_events (actor_user_id, action, resource_type, resource_id, metadata) VALUES ($1, $2, 'seller_application', $3, $4::jsonb)`,
        [req.user.id, `seller.application.${action}`, application.id, JSON.stringify({ from_status: application.status, to_status: target, user_id: application.user_id, note })]
      );
