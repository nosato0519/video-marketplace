  ...
  try {
    ...
    const result = await withTransaction(async (client) => {
      const current = await client.query(
        `SELECT id, user_id, status FROM seller_applications WHERE id = $1 FOR UPDATE`,
        [req.params.id]
      );
      if (!current.rowCount) {
        const error = new Error('seller_application_not_found');
        error.statusCode = 404;
        throw error;
      }
      const application = current.rows[0];
      if (!transitions[application.status]?.has(target)) {
        const error = new Error('invalid_seller_application_transition');
      ...
      const updated = await client.query(
        `UPDATE seller_applications
          ...
          WHERE id = $1
        RETURNING id, user_id, status, display_name, legal_name, country_code,
                  message, review_note, submitted_at, reviewed_at, reviewed_by, updated_at`,
        [application.id, target, note, req.user.id]
      );

      if (target === 'approved') {
        await client.query(
          `UPDATE users SET role = 'seller', updated_at = NOW() WHERE id = $1 AND role = 'buyer'`,
          [application.user_id]
        );
        await client.query(
      ...
      await client.query(
        `INSERT INTO audit_events (actor_user_id, action, resource_type, resource_id, metadata)
         VALUES ($1, $2, 'seller_application', $3, $4::jsonb)`,
