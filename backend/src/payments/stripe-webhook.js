...
export function createStripeWebhookHandler({
  ...
  stripe = secret ? new Stripe(process.env.STRIPE_SECRET_KEY || 'sk_test_placeholder') : null,
} = {}) {
  return async function handleStripeWebhook(req, res, next) {
    try {
      if (!secret || !stripe) return res.status(503).json({ error: { code: 'PAYMENT_PROVIDER_NOT_CONFIGURED' } });
      ...
      const result = await completePayment({ ...input, payment: { ...payload, order_id: payload.orderId, provider_payment_id: payload.paymentId } });
      return res.status(200).json({ received: true, result });
    } catch (error) {
      next(error);
    }
