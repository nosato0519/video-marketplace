...
- **2026-08-29:** Expanded `http-payment-refund-acceptance.js` to assert seller earnings are created as `available`, become exactly one `refunded` row with `refunded_at`, leave zero `available` balance after refund, and remain unchanged after a duplicate refund webhook.
...
### Verification status
- Latest refund-to-earnings acceptance code is on `main` at commit `ece8213a01ab2bdf3bc9190e84475dd4f131af7b`.
- No workflow run is currently returned for that commit, so the new refund earnings assertions are **not yet runtime-verified**.
...
### Exact checkpoint
Latest main commit: `ece8213a01ab2bdf3bc9190e84475dd4f131af7b`.
Previous corrected seller-earnings CI evidence exists for run `33245987373`; the refund correction and dedicated refund assertions post-date that run and therefore require a new CI run.

### Next exact task
1. Observe the push-triggered CI run for `ece8213a01ab2bdf3bc9190e84475dd4f131af7b` and record exact conclusion/failed step.
2. If CI fails, fix the exact refund acceptance or migration issue and rerun.
3. If CI passes, verify payout eligibility after refund including a real payout creation attempt.
4. Decide/implement accounting treatment for refunds after an earning has already been paid out.
5. Add/verify Checkout HTTP contract coverage for selected `providerId` passthrough.
6. Trace Stripe provider identity from Checkout metadata through webhook/event ledger and `completePayment`.
7. Continue payout runtime/CI verification and real non-Stripe adapter work.
