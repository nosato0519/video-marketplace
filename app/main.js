...
async function startCheckout(productId) {
  ...
  try {
    ...
    if (error.status === 401) { location.hash = `#/login?return=/product/${encodeURIComponent(productId)}`; return; }
  ...
  try {
    ...
    message.textContent = `Checkout ready (${intent.data.currency} ${intent.data.price}). Session: ${session.session.id || 'created'}`;
  } catch (error) {
    message.textContent = error.body?.error?.message || 'Unable to start checkout.';
    button.disabled = false;
  }
}
...
function renderProduct(id) {
  ...
  wireLocale();
  document.querySelector('#checkout')?.addEventListener('click', () => startCheckout(id));
}
